...
class _ViolationPageState extends State<ViolationPage>
      ...
      if (widget.initialData?.violations != null) {
        for (final violationModel in widget.initialData!.violations) {
          ...
          final violationKey = ViolationsConfig.definitions.entries
              .firstWhere(
                (entry) =>
                    entry.value.displayName == violationModel.violationName,
          ...
          context.read<ViolationBloc>().add(
                UpdateViolationEvent(
                  ...
                  value: {
                    ...
                    if (violationModel.selectedOption != null)
                      'option': violationModel.selectedOption,
                    if (violationModel.excessPassengers != null)
                      'excessPassengers': violationModel.excessPassengers,
                    if (violationModel.additionalDetails != null)
                      ...violationModel.additionalDetails!,
                  },
                ),
              );
        }
  ...
  bool _validateAllSteps(BuildContext context) {
    ...
    if (missingPersonalFields.isNotEmpty) {
      ...
      showAlert(context, 'Required Fields Missing',
    ...
    if (homeBlocState is HomeLoaded) {
      ...
      final hasSelectedViolations =
          homeBlocState.violations.values.any((value) => value != false);
    ...
    if (evidencePhoto == null ||
        ...
        evidencePhoto?.path == '') {
      showAlert(context, 'Evidence Photo Required',
          'Evidence photo is mandatory. Please capture or select a photo showing the violation.');
      return false;
    }
  ...
  Future<String?> submitForm(BuildContext context) async {
    ...
    if (isValid) {
      ...
      if (evidencePhoto == null ||
          ...
          evidencePhoto?.path == '') {
        showAlert(context, 'Evidence Photo Required',
            'Cannot submit report without evidence photo. Please capture a photo showing the violation.');
        setState(() {
          isSaving = false;
        });
        return null;
      ...
      try {
        ...
        if (widget.initialData != null &&
          ...
          if (currentUser != null) {
            final draftKey =
                'draft_${currentUser.uid}_${widget.initialData!.draftId!}';
        ...
        // Catch the exception from handleSave and show the specific error
        showAlert(
            context, 'Duplicate Violation', e.toString().replaceAll('Exception: ', ''));
        setState(() {
          isSaving = false;
        });
        return null;
  ...
  Future<void> navigateBackToHome() async {
    ...
    if (homeBlocState is HomeLoaded) {
      ...
      final hasData = report.fullname.trim().isNotEmpty ||
          ...
          report.plateNumber
              .trim()
              .isNotEmpty || // Include plate number in the check
      ...
      if (hasData) {
        ...
        final draftKeys =
            allKeys.where((key) => key.startsWith('draft_${userUUID}_')).toList();
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                AppTextFieldLabel(
                    label: "Select all that apply:", required: true),
                SizedBox(height: 16),
                ...
                EnhancedViolationItem(
                  ...
                  offenseNumber:
                      _getOffenseNumber('driving-without-carrying-license'),
                ),
                EnhancedViolationItem(
                ...
                EnhancedViolationItem(
                  ...
                  offenseNumber:
                      _getOffenseNumber('operating-without-franchise'),
                ),
                EnhancedViolationItem(
